...
CURRENT_TIME = datetime(2025, 6, 14, 20, 38)  # Updated to 08:38 PM CDT, June 14, 2025
...
theta = 4/3  # Polar angle (radians) as requested
...
CONFIG = {
    ...
    "max_iterations": 9,
    "time_delay_steps": 2,
    "ctc_feedback_factor": 2.72,  # Golden ratio, Pythagorean harmony
    "dt": 1e-12,
    "dx": l_p * 1e5,
    "dv": l_p * 1e3,
    "du": l_p * 1e3,
    ...
    "nugget_m": 1.618,
    "nugget_lambda": 2.72,
    "alpha_time": 3.183e-3,  # Increased to amplify displacement
...
def compute_entanglement_entropy(field, grid_size):
    ...
    if field.size == total_dim:
        ...
        # Compute Shannon entropy (base 2 for bits of information)
...
def compute_j6_potential(phi, j4, psi, ricci_scalar, kappa_j6, kappa_j6_eff, j6_scaling_factor):
    ...
    phi_term = np.tile((phi_flat[:4] / (phi_norm + 1e-10))**2, 81)
...
class NuggetFieldSolver3D:
    def __init__(self, grid_size=10, m=0.1, lambda_ctc=0.5, c=1.0, alpha=0.1, 
        ...
        self.theta = theta  # Use global theta for consistency
...
class Unified6DSimulation:
    ...
    def setup_symbolic_calculations(self):
        ...
        scaling_factor = (1 + sp.sqrt(5)) / 2  # Golden ratio, Pythagorean harmony
    ...
    def _compute_ctc_unitary_matrix(self):
        ...
        selected_indices = []
        for i in range(min(4, n_points // (n_points // 4))):
            start = i * (n_points // 4)
    ...
    def compute_metric_tensor(self):
        ...
        for i in range(self.total_points):
            # Pythagorean-weighted distance
            weights = self.metric_scale_factors
            r_weighted = self.compute_r_6D(coords[i:i+1])  # Compute for each point
    ...
    def compute_r_6D(self, coords):
        # Pythagorean-weighted 6D distance for a single point or array
    ...
    def compute_phi(self, coords):
        r_6D = self.compute_r_6D(coords)  # Should return array of shape (84,)
        k = CONFIG["k"]
        c_effective = CONFIG["c_prime"] if "c_prime" in CONFIG else c0
    ...
    def compute_time_displacement(self, u_entry, u_exit, v=0):
        ...
        c_effective = CONFIG["c_prime"] if "c_prime" in CONFIG else c0
        # Cyclical time modulation based on Metonic and Saros cycles
    ...
    def simulate_ctc_quantum_circuit(self):
        logger.info("Running CTC quantum circuit with vm-Teleportation (4 qutrits per node, 4 nodes)")
        num_qutrits_per_node = 4
        num_nodes = self.num_nodes
        dim_per_node = 3 ** num_qutrits_per_node
        ...
        phase = np.exp(1j * self.time * CONFIG["dt"] / hbar * (np.sin(2 * np.pi * self.time / METONIC_CYCLE) + np.sin(2 * np.pi * self.time / SAROS_CYCLE)))  # Dual cycle phase
        ...
        print(f"vm-Teleportation Circuit (4 qutrits per node, 4 nodes): Initial State |0000...0000>, H, Bell Entangled States, ACT2, CTC Feedback\n"
    ...
    def compute_V_j6(self):
        ...
        phi = self.compute_phi(self.lattice.coordinates)
        phi_324 = np.interp(np.linspace(0, 323, 324), np.arange(84), phi)
        j4 = np.sin(np.angle(self.quantum_state))
        j4_324 = j4
        psi = self.quantum_state
        psi_324 = psi
        r_6d = self.compute_r_6D(self.lattice.coordinates)
        ricci_scalar_324 = np.interp(np.linspace(0, 323, 324), np.arange(84), -G * m_n / (r_6d**4) * (1 / LAMBDA**2))
        logger.info(f"Projected shapes: phi_324={phi_324.shape}, ricci_scalar_324={ricci_scalar_324.shape}")
        V_j6 = compute_j6_potential(phi_324, j4_324, psi_324, ricci_scalar_324, CONFIG["kappa_j6"], 
    ...
    def evolve_quantum_state(self, dt):
        ...
        num_nodes = self.num_nodes
        dim_per_node = 81
        ctc_total_dim = num_nodes * dim_per_node
        ...
        if phi.shape != (84,):
            phi = phi.reshape(84)  # Ensure shape matches total_points
            logger.info(f"Reshaped phi to: {phi.shape}")
        g_tt = g_numeric[:, 0, 0]
        logger.info(f"g_tt shape before interpolation: {g_tt.shape}")
        if g_tt.shape != (84,):
            g_tt = g_tt.reshape(84)
            logger.info(f"Reshaped g_tt to: {g_tt.shape}")
        phi_proj_324 = np.interp(np.linspace(0, 323, 324), np.arange(84), phi)
        g_tt_proj_324 = np.interp(np.linspace(0, 323, 324), np.arange(84), g_tt)
        r_6d = self.compute_r_6D(coords)
        # Tetractys-based potential term
        ...
        if V_pot.shape != (84,):
            V_pot = V_pot.reshape(84)
            logger.info(f"Reshaped V_pot to: {V_pot.shape}")
        V_pot_proj_324 = np.interp(np.linspace(0, 323, 324), np.arange(84), V_pot)
        V_j6 = self.compute_V_j6()
        logger.info(f"V_j6 shape before interpolation: {V_j6.shape}")
        if V_j6.shape != (324,):
            V_j6 = V_j6.reshape(324)
            logger.info(f"Reshaped V_j6 to: {V_j6.shape}")
        V_j6_proj_324 = V_j6
        # Apply harmonic ratio to V_total
        harmonic_weights = self.metric_scale_factors / np.sum(self.metric_scale_factors)  # Normalized Fibonacci weights
        V_total = V_pot_proj_324 + V_j6_proj_324 + 1e-20 * g_tt_proj_324 * harmonic_weights[0] + 1e-4 * phi_proj_324 * harmonic_weights[1]
